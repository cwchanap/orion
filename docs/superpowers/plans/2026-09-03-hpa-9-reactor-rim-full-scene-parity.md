...
This validates the booted simulator's **logical viewport** against the 390×844 contract before capturing: it resolves the booted device's `deviceTypeIdentifier` to its `.simdevicetype` profile and reads the declared screen geometry (points = pixels ÷ `mainScreenScale`), so a 393×852-point model (iPhone 16/17-class) is refused instead of being normalized into a false-positive evidence row. The capture itself must then match the panel's native portrait pixels before `sips` normalizes it to the 390×844 output size. Boot a 390×844-point simulator (iPhone 14-class or iPhone 16e) before invoking it; each scene task owns the navigation instructions that put the real app in the representative state.
